...
def create_rxn_Morgan2FP_separately(rsmi, psmi, rxnfpsize=16384, pfpsize=16384, useFeatures=False, calculate_rfp=True,
                                    useChirality=False):
...
class NeuralNetContextRecommender():
    ...
    def load_nn_model(self, model_path="", info_path="", weights_path=""):
        ...
        self.s2_func = K.function([fp_transform_layer.output, c1_input_layer.output, s1_input_layer.output],
                                  [s2_output.output])
        self.r1_func = K.function(
            [fp_transform_layer.output, c1_input_layer.output, s1_input_layer.output, s2_input_layer.output],
            [r1_output.output])
        self.r2_func = K.function(
            [fp_transform_layer.output, c1_input_layer.output, s1_input_layer.output, s2_input_layer.output,
             r1_input_layer.output], [r2_output.output])
        self.T_func = K.function(
            [fp_transform_layer.output, c1_input_layer.output, s1_input_layer.output, s2_input_layer.output,
             r1_input_layer.output, r2_input_layer.output], [T_output.output])
    ...
    def predict_top_combos(self, inputs, return_categories_only=False, c1_rank_thres=2, s1_rank_thres=3,
                           s2_rank_thres=1, r1_rank_thres=3, r2_rank_thres=1):
                        ...
                        for r2_cdt in r2_cdts:
                            ...
                            ## esle ouptupt format compatible with the overall framework
                                ...
                                context_combos.append(
                                    [float(T_pred[0][0][0]), '.'.join(slv_name), '.'.join(rgt_name), '.'.join(cat_name),
                                     np.nan, np.nan])
...
def condition_pred(smiles) :
    def load_nn_model(model_name="aizynthfinder/utils/model.json", info_folder="aizynthfinder/utils/", weights_name="aizynthfinder/utils/weights.h5"):
        # 获取当前文件夹路径
        current_dir = os.getcwd()

        # 构建完整的模型文件路径
        model_path = os.path.join(current_dir, model_name)
        info_path = os.path.join(current_dir, info_folder)
        weights_path = os.path.join(current_dir, weights_name)

        # 调用函数，并传入构建好的路径
        cont.load_nn_model(model_path=model_path, info_path=info_path, weights_path=weights_path)


    cont = NeuralNetContextRecommender()

    '''cont.load_nn_model(model_path="C:/Users/77936/Desktop/NeuralNet_Cont_Model/model.json",
                       info_path="C:/Users/77936/Desktop/NeuralNet_Cont_Model/",
                       weights_path="C:/Users/77936/Desktop/NeuralNet_Cont_Model/weights.h5")'''
    '''print(cont.get_n_conditions('COC(OC)OC.Cc1cccc(C)c1N(CC(=O)Nc1ccc(C(N)=NO)cc1)C(=O)C1CCS(=O)(=O)CC1>>Cc1cccc(C)c1N(CC(=O)Nc1ccc(-c2ncon2)cc1)C(=O)C1CCS(=O)(=O)CC1', 1, with_smiles=False,
                                return_scores=True))'''
    load_nn_model()
    conditions = cont.get_n_conditions(smiles, 1, with_smiles=False,return_scores=True)
    ans = f'Temperature:{conditions[0][0][0]}\nsolvent:{conditions[0][0][1]}\ncatalyzer:{conditions[0][0][2]}'
    return ans